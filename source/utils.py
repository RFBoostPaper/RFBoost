...
def get_acf(csi_data, channel_gain, samp_rate = 1000, window_size = 256, nfft=1000, window_step = 10, agg_type = 'pca', cache_folder=None):
    if agg_type == "pca":
...
def get_dfs(csi_data, channel_gain, samp_rate = 1000, window_size = 256, nfft=1000, window_step = 10, agg_type = 'ms', cache_folder=None):
    """
    ...
    try:
        ...
        freq_lpf_negative_min = 60
        
        # doppler_spectrum = np.zeros((csi_data.shape[0], 1 + freq_lpf_positive_max + freq_lpf_negative_min,
        #                         int(csi_data.shape[0] / window_step)))
        ...
        if agg_type == 'pca' and csi_data.shape[1] >= 1:
            ...
            # [T,1]
            csi_data_agg = np.dot(csi_data, pca_coef[:, 0])
        elif agg_type == 'ms' and channel_gain is not None:
            csi_data_agg = csi_data
        ...
        # csi_data_agg = np.transpose(csi_data_agg, [1,0])
        ...
        # print(freq_time_prof_allfreq.shape)
...
def show_dfs(f, t, Zxx, ax):
    ax.pcolormesh(t, f, np.abs(Zxx), cmap = 'jet')
